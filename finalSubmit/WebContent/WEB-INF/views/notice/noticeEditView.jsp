...
<body>
	...
	<!-- Contact Section Heading-->
		...
		style="font-size: 40px;">공지사항 수정</p>
			...
			<form:form id="noticeForm"
				...
				<div class="form-control-lg">
					...
					<select name="noticeCategory" id="noticeCategory"
						style="width: 70px; height: 25px; font-size: 16px;">
						<option value="알림"
							${notice.noticeCategory == '알림' ? 'selected' : ''}>알 림</option>
						<option value="이벤트"
							${notice.noticeCategory == '이벤트' ? 'selected' : ''}>이벤트</option>
					</select> </select>
				</div>
				<div class="mb-5 form-control-lg">
					<label for="exampleFormControlInput1" class="form-label">제목
						...
						id="exampleFormControlInput1" value="${notice.noticeTitle }">
				</div>
				
				<input name="noticeNo" type="hidden" value="${notice.noticeNo }" />
				<textarea class="form-control" name="noticeContent"
					id="noticeContent" rows="20" cols="10"
					style="height: 500px; display: none;">${notice.noticeContent}</textarea>
					<table>
					<tr>
						...
						function() {
							...
							$(".file_area")
									.append(
											'<div class="file_div">'
													+ '<input type="file" name="boFiles" />'
													+ '<button type="button" class="btn btn-outline-primary btn-sm btn_delete">삭제</button>'
													+ '</div>');
						});
